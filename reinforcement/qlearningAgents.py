...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeValueFromQValues(self, state):
        ...
        if (maxaction == None):
            return 0.0
        result = self.getQValue(state, maxaction)
        return result
        #util.raiseNotDefined()
    ...
    def update(self, state, action, nextState, reward: float):
        ...
        """
        "*** YOUR CODE HERE ***"
        firstTerm = (1.0 - self.alpha) * self.qvalues[(state, action)]
        sample = reward + self.discount * self.computeValueFromQValues(nextState)
        secondTerm = self.alpha * sample
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def getQValue(self, state, action):
        ...
        for feature in featuresvector.keys():
            sum = sum + (self.weights[feature] * featuresvector[feature])
        return sum
    ...
    def update(self, state, action, nextState, reward: float):
        ...
        "*** YOUR CODE HERE ***"
        featuresvector = self.featExtractor.getFeatures(state, action)
        firstTerm = reward + (self.discount * self.computeValueFromQValues(nextState))
        secondTerm = self.getQValue(state, action)
